...
const _uuid = Uuid();
// число вопросов - фиксированное свойство самой методики ASRS-v1.1, не
// зависит от языка. Инициализатор поля класса ниже выполняется ДО того,
// как появляется BuildContext, поэтому asrsQuestions(l10n).length здесь
// физически недоступен - нужна обычная числовая константа (тот же приём,
// что в phq9_screen.dart/gad7_screen.dart)
const _asrsQuestionCount = 6;
...
class _AsrsScreenState extends State<AsrsScreen> {
  ...
  List<int?> _currentAnswers = List.filled(_asrsQuestionCount, null);
  ...
  void _startTest() {
    setState(() {
      _currentAnswers = List.filled(_asrsQuestionCount, null);
      _currentQuestion = 0;
      _mode = _Mode.testing;
    });
  }
  ...
  Widget _buildTest() {
    ...
    final isLast = index == _asrsQuestionCount - 1;
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          ...
          children: List.generate(_asrsQuestionCount, (i) {
  ...
  Widget _buildQuestion(int index) {
    final l10n = AppLocalizations.of(context)!;
    final questions = asrsQuestions(l10n);
    final responseLabels = asrsResponseLabels(l10n);
    return GlassPanel(
      opacity: 0.08,
      blurred: false,
            ...
            children: [
              ...
              Text(
                result.suggestsFurtherAssessment
                    ? l10n.asrsSuggestAssessment
                    : l10n.asrsNoAssessmentNeeded,
                textAlign: TextAlign.center,
                style: TextStyle(
        ...
        if (widget.onDiscussWithAi != null) ...[
          ...
          Material(
            ...
            child: InkWell(
              borderRadius: BorderRadius.circular(14),
              onTap: () async {
                final text = l10n.asrsDiscussPrompt(
                  result.shadedCount,
                  result.suggestsFurtherAssessment
                      ? l10n.who5RecommendSpecialist
                      : '',
                );
