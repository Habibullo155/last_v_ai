...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    super.initState();
    // при каждом открытии экрана чата - если pro-квота СЕЙЧАС доступна
    // (новый месяц или только что оформили подписку), старые
    // уведомления "закончились pro-токены" на прошлых ответах уже
    // неактуальны - прячем сразу во всех переписках (см.
    // ChatStore.clearStaleModelDowngradedNotices), не дожидаясь
    // следующего ответа модели
    unawaited(_checkAndClearStaleDowngradeNotices());
  }

  Future<void> _checkAndClearStaleDowngradeNotices() async {
    final token = widget.authStore.token;
    if (token == null) return;
    try {
      final usage = await _usageService.getMyUsage(
        baseUrl: widget.authStore.baseUrl,
        token: token,
      );
      if (usage.proAvailableNow) {
        widget.store.clearStaleModelDowngradedNotices();
      }
    } on UsageException {
      // не критично - уведомления просто останутся до следующей успешной
      // проверки (следующее открытие экрана, или естественным образом
      // пропадут вместе с ответом, где квота учтётся заново)
    }
  }
    ...
    try {
      ...
      if (sounds.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context)!.chatNoSoundsUploaded),
          ),
        );
        return;
      }
                ...
                children: [
                  if (_playingNoiseId != null)
                    ListTile(
                      ...
                      title: Text(
                        AppLocalizations.of(context)!.chatStopButton,
                        style: const TextStyle(color: Colors.white),
                      ),
                      onTap: () =>
                          Navigator.of(context).pop(_stopNoiseSentinel),
                    ),
                  ...sounds.map(
                    (s) => ListTile(
                      leading: Icon(
  ...
  String _precedingUserMessage(List<ChatMessage> messages, int assistantIndex) {
    ...
    return l10n.chatQuestionNotFound;
  }
  ...
  String? _recentChatSnapshot() {
    ...
    final l10n = AppLocalizations.of(context)!;
    final recent = messages.length > 15
    ...
    switch (test) {
      case 'phq9':
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) =>
                Phq9Screen(userId: userId, onDiscussWithAi: discuss),
          ),
        );
        break;
      case 'gad7':
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) =>
                Gad7Screen(userId: userId, onDiscussWithAi: discuss),
          ),
        );
        break;
      case 'asrs':
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) =>
                AsrsScreen(userId: userId, onDiscussWithAi: discuss),
          ),
        );
        break;
      case 'custom':
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => CustomTestListScreen(
              authStore: widget.authStore,
              onDiscussWithAi: discuss,
            ),
          ),
        );
        break;
  ...
  Future<void> _requestLiveHelp() async {
              ...
              children: [
                ...
                Center(
                  child: TextButton(
                    ...
                    child: Text(
                      l10n.commonCancel,
                      style: TextStyle(color: context.onSurfaceFaded(0.5)),
                    ),
                  ),
                ),
              ],
    ...
    if (choice == 'share' && snapshot != null) {
      ...
      try {
        ...
        final trimmed = recentMessages.length > 15
        ...
        return Column(
          children: [
            _buildTopBar(
              context,
              showSidebar,
              convo?.title ?? l10n.chatNewChatTitle,
            ),
            if (widget.authStore.user?.isEmailVerified == false)
              _buildVerifyEmailBanner(),
            const SizedBox(height: 12),
            Expanded(
              child: Center(
                child: ConstrainedBox(
                  ...
                  child: messages.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, i) => MessageBubble(
                            message: messages[i],
                            onDelete: () =>
                                widget.store.deleteMessage(messages[i].id),
                            onEdit: messages[i].role == MessageRole.user
                                ? (newText) async {
                                    await widget.store.editAndResend(
                                      messages[i].id,
                                      newText,
                                    );
                                    _scrollToBottom();
                            ...
                            onReport: messages[i].role == MessageRole.assistant
                                ? () => showReportDialog(
                                    context,
                                    authStore: widget.authStore,
                                    userMessage: _precedingUserMessage(
                                      messages,
                                      i,
                                    ),
                            ...
                            onTestPromptResponse: messages[i].offersTestPrompt
                                ? (yes) async {
                                    setState(
                                      () =>
                                          messages[i].testPromptAnswered = true,
                                    );
                                    if (yes) {
                                      await _chooseTest();
                                    } else {
                            ...
                            onThemePickerDismissed:
                                messages[i].offersThemePicker
                                ? () => setState(
                                    () =>
                                        messages[i].themePickerAnswered = true,
                                  )
                                : null,
                            onWatchAd: messages[i].dailyLimitReason != null
                                ? () async {
                                    final ready = await _adService.preload(
                                      widget.authStore.baseUrl,
                                    );
                                    if (!ready) return false;
                                    final earned = await _adService
                                        .showAndWaitForReward();
                                    if (!earned) return false;
                                    final token = widget.authStore.token;
                                    if (token == null) return false;
                                    await _telegramService.confirmAdWatched(
                                      baseUrl: widget.authStore.baseUrl,
                                      token: token,
                                    );
                                    // подгружаем следующий ролик заранее, не ждём
                                    // следующего нажатия - тот же приём, что в purchase_screen.dart
                                    unawaited(
                                      _adService.preload(
                                        widget.authStore.baseUrl,
                                      ),
                                ...
                                : null,
                            onLinkTelegram: messages[i].dailyLimitReason != null
                                ? () async {
                                    final token = widget.authStore.token;
                                    if (token == null) return;
                                    try {
                                      final info = await _telegramService
                                          .startLink(
                                            baseUrl: widget.authStore.baseUrl,
                                            token: token,
                                          );
                                      ...
                                      await showDialog(
                                        context: context,
                                        builder: (context) =>
                                            TelegramCodeDialog(info: info),
                                      );
                                    } on TelegramException catch (_) {
                                      // карточка в чате - не место для длинного текста
                                      // ошибки, человек попробует снова или уйдёт в
                                      // раздел подписки сам, где есть место для деталей
                                    }
                                  }
                                : null,
            ...
            Text(
              ...
              style: TextStyle(
                color: context.onSurfaceFaded(0.28),
                fontSize: 10.5,
              ),
            ),
          ],
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          onTap: () => Navigator.of(context).push(
            MaterialPageRoute(
              builder: (_) => VerifyEmailScreen(store: widget.authStore),
            ),
          ),
          child: Container(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            tooltip: _playingNoiseId != null
                ? l10n.chatMuteBackgroundSound
                : l10n.chatUnmuteBackgroundSound,
      ...
      onSelected: (value) {
        switch (value) {
          case 'profile':
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => ProfileScreen(authStore: widget.authStore),
              ),
            );
            break;
          case 'wellbeing':
          ...
          case 'purchase':
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => PurchaseScreen(authStore: widget.authStore),
              ),
            );
            break;
          case 'settings':
          ...
          case 'my_reports':
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => MyReportsScreen(authStore: widget.authStore),
              ),
            );
            break;
          case 'my_help':
          ...
          case 'operator':
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) =>
                    OperatorDashboardScreen(authStore: widget.authStore),
              ),
            );
            break;
          case 'support':
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => SupportScreen(authStore: widget.authStore),
              ),
            );
            break;
          case 'blog':
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => BlogListScreen(authStore: widget.authStore),
              ),
            );
            break;
          case 'sleep_music':
          ...
          children: [
            ...
            Wrap(
              ...
              children: suggestions
                  .map(
                    (s) =>
                        _SuggestionChip(text: s, onTap: () => onPromptTap(s)),
                  )
                  .toList(),
            ),
          ],
...
enum _AiMode {
  ...
  grief(icon: Icons.music_note_outlined),
