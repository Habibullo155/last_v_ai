...
class _PurchaseScreenState extends State<PurchaseScreen>
  ...
  void initState() {
    ...
    _load();
    _adService.initialize().then(
      (_) => _adService.preload(widget.authStore.baseUrl),
    );
  }
  ...
  Future<void> _loadTelegramStatus() async {
    final token = widget.authStore.token;
    if (token == null) return;
    final linked = await _telegramService.isLinked(
      baseUrl: widget.authStore.baseUrl,
      token: token,
    );
  ...
  Future<void> _startTelegramLink() async {
    ...
    if (token == null) return;
    try {
      final info = await _telegramService.startLink(
        baseUrl: widget.authStore.baseUrl,
        token: token,
      );
      if (!mounted) return;
      await showDialog(
        context: context,
        builder: (context) => TelegramCodeDialog(info: info),
      );
      _loadTelegramStatus();
  ...
  Future<void> _watchAd() async {
    ...
    try {
      ...
      final token = widget.authStore.token;
      if (token == null) return;
      final remaining = await _telegramService.confirmAdWatched(
        baseUrl: widget.authStore.baseUrl,
        token: token,
      );
  ...
  Future<void> _buy(BillingPlan plan) async {
    ...
    try {
      ...
      if (!opened && mounted) {
        setState(
          () =>
              _error = AppLocalizations.of(context)!.purchaseCheckoutOpenFailed,
        );
      }
    } on BillingException catch (e) {
      if (mounted) setState(() => _error = e.message);
  ...
  Widget _buildContent() {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          l10n.purchaseAvailablePlansSection,
          style: TextStyle(
        ...
        if (_plans.isEmpty)
          Padding(
            ...
            child: Text(
              l10n.purchaseNoPlansConfigured,
      ...
      child: Column(
        ...
        children: [
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        color: _isTelegramLinked
                            ? context.onSurfaceFaded(0.08)
                            : null,
                      ),
                      child: Text(
                        _isTelegramLinked
                            ? l10n.purchaseTelegramLinked
                            : l10n.purchaseTelegramLinkButton,
                        style: TextStyle(
                          color: _isTelegramLinked
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          borderRadius: BorderRadius.circular(12),
                          border: Border.all(
                            color: context.onSurfaceFaded(0.2),
                          ),
                        ),
                        child: _isWatchingAd
                            ? SizedBox(
          ...
          if (_adBonusRemaining != null) ...[
            ...
            Text(
              l10n.purchaseAdBonusRemaining(_adBonusRemaining!),
              style: TextStyle(
        ...
        children: [
          ...
          Text(
            usage.tokensLimit == null
                ? l10n.purchaseTokensUsedNoLimit(usage.tokensUsedThisMonth)
                : l10n.purchaseTokensUsedWithLimit(
          ...
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    plan.isFree
                        ? l10n.purchaseFreeDuration(plan.durationDays)
                        : l10n.purchasePriceDuration(
                            plan.priceRubDisplay,
            ...
            if (!plan.isFree)
              Material(
                ...
                child: InkWell(
                  borderRadius: BorderRadius.circular(12),
                  onTap: plan.isPurchasable && !isBusy
                      ? () => _buy(plan)
                      : null,
                  child: Container(
                    padding: const EdgeInsets.symmetric(
