...
class _VoiceSettingsScreenState extends State<VoiceSettingsScreen> {
      ...
      return GlassPanel(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              l10n.voiceSettingsDisabledByAdmin,
              style: TextStyle(
  ...
  Widget _buildVoiceOptions(VoiceStore voice) {
    final femaleVoices = voice.availableVoices
        .where((v) => _guessGender(v['name'] ?? '') == _VoiceGender.female)
        .toList();
    final maleVoices = voice.availableVoices
        .where((v) => _guessGender(v['name'] ?? '') == _VoiceGender.male)
        .toList();
    final otherVoices = voice.availableVoices
        .where((v) => _guessGender(v['name'] ?? '') == _VoiceGender.other)
        .toList();
    final l10n = AppLocalizations.of(context)!;
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        _sectionLabel(l10n.voiceSettingsSpeechRecognitionSection),
        GlassPanel(
          opacity: 0.08,
          borderRadius: BorderRadius.circular(18),
          padding: const EdgeInsets.all(16),
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  voice.isSttAvailable
                      ? l10n.voiceSettingsMicAvailable
                      : l10n.voiceSettingsMicUnavailable,
                  style: TextStyle(
                    color: context.onSurfaceFaded(0.6),
                    fontSize: 12.5,
                  ),
        ...
        _sectionLabel(l10n.voiceSettingsAutoReadSection),
        GlassPanel(
          opacity: 0.08,
          borderRadius: BorderRadius.circular(18),
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
          child: Row(
            children: [
              Expanded(
                child: Text(
                  l10n.voiceSettingsAutoReadToggle,
                  style: TextStyle(color: context.onSurface),
                ),
              ),
              Switch(
        ...
        if (voice.isCloudTtsAvailable) ...[
          const SizedBox(height: 20),
          _sectionLabel(l10n.voiceSettingsVoiceSection),
          Text(
            l10n.voiceSettingsVoiceInstructions,
            style: TextStyle(
              color: context.onSurfaceFaded(0.4),
              fontSize: 11.5,
            ),
          ...
          Text(
            l10n.voiceSettingsNoVoicesLoaded,
            style: TextStyle(
              color: context.onSurfaceFaded(0.4),
              fontSize: 12.5,
            ),
          ...
          if (femaleVoices.isNotEmpty) ...[
            const SizedBox(height: 20),
            _sectionLabel(l10n.voiceSettingsFemaleSection),
            ...femaleVoices.map((v) => _buildVoiceTile(voice, v)),
          ],
          if (maleVoices.isNotEmpty) ...[
            const SizedBox(height: 20),
            _sectionLabel(l10n.voiceSettingsMaleSection),
            ...maleVoices.map((v) => _buildVoiceTile(voice, v)),
          ],
          if (otherVoices.isNotEmpty) ...[
            const SizedBox(height: 20),
            _sectionLabel(l10n.voiceSettingsOtherSection),
            Text(
              l10n.voiceSettingsCouldNotClassify,
              style: TextStyle(
  ...
  Widget _buildCloudVoiceTile(VoiceStore voice, CloudVoice v) {
    ...
    return Padding(
      ...
      child: GlassPanel(
        ...
        blurred:
            false, // список из нескольких голосов — см. message_bubble.dart
        ...
        child: Material(
          ...
          child: InkWell(
            ...
            onTap: () => voice.updateSettings(
              voice.settings.copyWith(cloudVoiceName: v.name),
            ),
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    ...
                    tooltip: l10n.voiceSettingsPreviewTooltip,
  ...
  Widget _buildVoiceTile(VoiceStore voice, Map<String, String> v) {
    final l10n = AppLocalizations.of(context)!;
    final selected =
        v['name'] == voice.settings.voiceName &&
        v['locale'] == voice.settings.voiceLocale;
    return Padding(
      padding: const EdgeInsets.only(bottom: 8),
      child: GlassPanel(
        opacity: selected ? 0.14 : 0.07,
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    ...
                    tooltip: l10n.voiceSettingsPreviewTooltip,
                    onPressed: () =>
                        voice.previewVoice(v['name'] ?? '', v['locale'] ?? ''),
                  ),
                  if (selected)
                    const Icon(
                      Icons.check_circle_rounded,
