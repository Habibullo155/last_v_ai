...
class ChatStore extends ChangeNotifier {
  ...
  // вызывается при открытии экрана чата (см. chat_screen.dart) - если
  // pro-квота СЕЙЧАС доступна (новый месяц или только что оформили
  // подписку), старые уведомления "закончились pro-токены" на прошлых
  // ответах уже неактуальны - прячем их сразу во ВСЕХ переписках, не
  // дожидаясь, пока человек долистает до каждой из них вручную. Сам
  // текст ответа не трогаем - только флаг показа уведомления
  void clearStaleModelDowngradedNotices() {
    var changed = false;
    for (final convo in conversations) {
      for (final message in convo.messages) {
        if (message.modelDowngradedReason != null) {
          message.modelDowngradedReason = null;
          changed = true;
        }
      }
    }
    // не вызываем _persist() - modelDowngradedReason намеренно не
    // сериализуется вообще (переходное поле, как dailyLimitReason), так
    // что сохранённому на диске JSON эта правка ничего не меняет -
    // нужен только notifyListeners(), чтобы перерисовать открытый экран
    ...
    try {
      ...
      await for (final event in _api.sendMessage(
        ...
        if (event.error != null) {
          ...
          if (event.dailyLimitReason != null) {
            // не помечаем как ошибку - это ожидаемый сценарий ("кончились
            // бесплатные запросы, вот как получить ещё"), не сбой, текст
            // не должен звучать тревожным красным цветом (см. isError
            // ниже в message_bubble.dart)
            assistantMsg.dailyLimitReason = event.dailyLimitReason;
          } else {
        ...
        if (event.modelDowngradedReason != null) {
          // служебное событие без текста (token: '', done: false) -
          // только донести причину до сообщения, не добавлять в
          // содержимое и не завершать поток
          assistantMsg.modelDowngradedReason = event.modelDowngradedReason;
        }
