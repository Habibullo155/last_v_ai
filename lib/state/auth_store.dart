...
/// AuthStore - обычный ChangeNotifier, не виджет, своего BuildContext не
/// имеет - получает его через глобальный navigatorKey (navigation.dart),
/// не протягивая AppLocalizations через параметр каждого метода во всех
/// вызывающих экранах. Резервный вариант (e.toString(), не локализован)
/// - только на случай, если ни один экран ещё не отрисовался, что на
/// практике не должно происходить к моменту вызова методов авторизации.
String _localizedAuthError(auth_svc.AuthException e) {
  final l10n = currentL10n();
  return l10n != null ? e.localizedMessage(l10n) : e.toString();
}
...
class AuthStore extends ChangeNotifier {
  ...
  Future<String?> enableBiometric() async {
    ...
    if (!supported) {
      return currentL10n()?.authBiometricNotConfigured ??
    ...
    final confirmed = await _biometricService.authenticate(
      reason: currentL10n()?.authBiometricConfirmReason ?? 'Подтверди, чтобы включить вход по биометрии',
    );
    if (!confirmed) return currentL10n()?.authBiometricConfirmFailed ?? 'Не удалось подтвердить — попробуй ещё раз.';
  ...
  Future<bool> forgotPassword(String email) async {
    ...
    try {
      ...
      lastError = _localizedAuthError(e);
      return false;
    } catch (e) {
      lastError = currentL10n()?.authUnexpectedErrorGeneric('$e') ?? 'Непредвиденная ошибка: $e';
      return false;
    } finally {
      isBusy = false;
      notifyListeners();
  ...
  Future<String?> deleteAccount(String password) async {
    final currentToken = token;
    if (currentToken == null) return currentL10n()?.authSessionNotFound ?? 'Сессия не найдена. Войди заново.';
    ...
    try {
      ...
      return _localizedAuthError(e);
    } catch (e) {
      return currentL10n()?.authUnexpectedErrorGeneric('$e') ?? 'Непредвиденная ошибка: $e';
    } finally {
      isBusy = false;
      notifyListeners();
    }
