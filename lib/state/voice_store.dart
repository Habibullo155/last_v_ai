...
class VoiceStore extends ChangeNotifier {
  ...
  Future<void> previewVoice(String name, String locale) async {
    ...
    try {
      ...
      isSpeaking = true;
      notifyListeners();
      await _tts.speak(currentL10n()?.voicePreviewPhrase ?? 'Привет! Так звучит этот голос.');
    } catch (_) {
      isSpeaking = false;
      notifyListeners();
    } finally {
  ...
  Future<void> previewCloudVoice(String voiceName) async {
    if (!isVoiceAvailable || !isCloudTtsAvailable) return;
    await _speakCloud(currentL10n()?.voicePreviewPhrase ?? 'Привет! Так звучит этот голос.', voiceOverride: voiceName);
  }
