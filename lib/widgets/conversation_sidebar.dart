        ...
        children: [
          ...
          _SidebarLinkTile(
            ...
            onTap: () {
              ...
              // закрываем меню ПЕРЕД переходом, не после - иначе pop()
              // из onSelected снимал бы только что открытый экран
              // календаря (тот же Navigator, только что положили его
              // route наверх), а не само меню
              onSelected?.call();
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => WellbeingCalendarScreen(
                    userId: userId,
                    authStore: authStore,
                  ),
                ),
          ...
          _SidebarLinkTile(
            ...
            onTap: () {
              ...
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) =>
                      WellbeingScreen(userId: userId, authStore: authStore),
                ),
              );
            },
          ),
          ...
          if (onCallHelp != null) ...[
            ...
            _SidebarLinkTile(
              ...
              onTap: () {
                // тот же порядок, что и у Календаря/Самочувствия выше:
                // _requestLiveHelp асинхронна и открывает диалог через
                // showDialog - если сначала вызвать её, а потом pop() из
                // onSelected, pop() закрыл бы только что открывшийся
                // диалог, а не само меню (onCallHelp!.call() не
                // дожидается диалога, просто запускает его)
                onSelected?.call();
                onCallHelp!.call();
              },
            ),
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return _ConversationTile(
                      ...
                      onRename: (newTitle) =>
                          store.renameConversation(convo.id, newTitle),
                    );
                  },
                );
              },
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(14),
            color: isActive ? context.onSurfaceFaded(0.14) : Colors.transparent,
          ),
          child: Row(
            children: [
              Expanded(
