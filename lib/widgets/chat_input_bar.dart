...
class _ChatInputBarState extends State<ChatInputBar> {
  ...
  /// сообщения - браузер по политике безопасности часто отказывается
  ...
  ///
  /// ВАЖНО: unfocus() и requestFocus() здесь СИНХРОННО, без await/delay
  /// между ними - мобильные браузеры (подтверждено и на Android Chrome,
  /// не только iOS Safari) показывают клавиатуру ТОЛЬКО если focus()
  /// вызван синхронно внутри самого обработчика пользовательского жеста;
  /// малейшая асинхронная пауза (даже Future.delayed на 50мс, как было
  /// здесь раньше) рвёт эту связь с жестом, и браузер отказывается
  /// показывать клавиатуру вообще - именно это и было причиной проблемы
  /// после первого исправления, не сам факт использования unfocus().
          ...
          Row(
            ...
            child: KeyboardListener(
              ...
              child: TextField(
                ...
                // ВСЕГДА true, намеренно НЕ widget.enabled - см.
                // flutter/flutter#192544 (открыт совсем недавно, ещё не
                // исправлен на момент написания): переключение
                // TextField.enabled на вебе внутри маппится на readOnly,
                // а смена readOnly на УЖЕ ЖИВОМ соединении с платформенным
                // текстовым вводом НАВСЕГДА ломает это соединение - клавиатура
                // просто перестаёт появляться при следующем тапе, даже
                // если фокус технически есть. У нас enabled переключался
                // именно так: false пока ИИ отвечает, true когда ответ
                // пришёл - ровно тот паттерн, что ломает соединение.
                // Разблокировка отправки логически не требует блокировки
                // самого поля - см. проверку !widget.enabled в _submit()
                // ниже и отдельно заблокированную кнопку отправки
                // (_SendButton) - печатать/читать можно всегда, отправить
                // повторно, пока ИИ уже отвечает - нельзя.
                enabled: true,
