...
class _AnimatedAppBackgroundState extends State<_AnimatedAppBackground> {
  ...
  // 10-секундном цикле 8 обновлений в секунду всё ещё выглядят достаточно
  // плавно для человеческого глаза, а нагрузка на основной поток заметно
  // ниже, чем при тикере на полной частоте экрана (обычно 60 раз/сек).
  //
  // Раньше цикл был 24 секунды - при такой скорости движение почти не
  // считывалось глазом как "движение", а воспринималось как медленное
  // подвисание/дрожание, что оказалось неприятнее, чем спокойное, но
  // отчётливо заметное кружение. 10 секунд - заметно быстрее (в 2.4
  // раза), но всё ещё плавно: угловой шаг за одно обновление вырос с
      ...
      builder: (context, _) {
        ...
        return Stack(
          ...
          children: [
            ...
            // RepaintBoundary - эта анимация крутится непрерывно (10 сек
                ...
                builder: (context, constraints) {
                  ...
                  return ValueListenableBuilder<double>(
                    ...
                    builder: (context, progress, _) {
                      ...
                      // раньше у второго и третьего блика были РАЗНЫЕ
                      // частоты по x и y (например sin(t*0.7) против
                      // cos(t)) - это давало фигуру Лиссажу с точками
                      // кажущегося разворота и неравномерного замедления,
                      // а не гладкое кружение. Теперь у всех трёх - одна
                      // и та же частота t по обеим осям (чистый эллипс,
                      // как уже было у первого блика), только со сдвигом
                      // фазы на треть оборота (~2.1 рад) друг относительно
                      // друга, чтобы блики не сходились в одной точке
                      // одновременно. На нагрузку это не влияет - sin/cos
                      // считаются одинаково быстро в любом случае, дело
                      // только в том, как это ощущается на глаз
                      return Stack(
                        children: [
                          _blob(
                            color: palette.blob1,
            ...
            // тёмный режим: тёмно-зелёная плёнка притушивает блики, чтобы
            // они не резали глаз на тёмном фоне. Светлый режим - лёгкая
            // голубоватая плёнка - смягчает края бликов, не вымывает цвет
            // целиком. При reducedContrast обе плёнки усилены - именно это
            // и означает "снизить контрастность". Конкретные оттенки (не
            // просто белый/чёрный) подобраны для более приятного вида.
            if (isLight)
              Container(
