...
class ChatMessage {
  ...
  // "telegram_needed" | "ad_needed" | null - заполняется из заголовка
  // X-Daily-Limit-Reason на 402 (см. chat_api_service.dart), когда
  // дневной лимит бесплатных запросов исчерпан. Не сохраняется между
  // сессиями по той же причине, что offersTestPrompt - к следующему
  // открытию чата лимит мог уже обновиться (новый день, бонус применён),
  // и кнопки под старым сообщением были бы не актуальны
  String? dailyLimitReason;
  // true после того, как человек успешно посмотрел рекламу или привязал
  // Telegram прямо из этой карточки - прячем кнопки под этим конкретным
  // сообщением, не удаляя сам факт из истории (тот же приём, что
  // testPromptAnswered)
  bool dailyLimitResolved;
  // "pro_quota_exhausted" | null - из заголовка X-Model-Downgraded на
  // УСПЕШНОМ ответе (см. chat_api_service.dart) - в отличие от
  // dailyLimitReason выше, это не отказ, ответ уже пришёл, просто другой
  // моделью. Не сохраняется между сессиями по той же причине - к
  // следующему месяцу квота обновится, старое уведомление будет неактуально
  String? modelDowngradedReason;
