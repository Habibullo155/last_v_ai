class UsageInfo {
  ...
  // отправь сейчас новое сообщение - ушло бы pro-моделью? Та же логика,
  // что реально используется на бэкенде при генерации ответа (не
  // отдельная копия) - используется, чтобы спрятать старые уведомления
  // "закончились pro-токены" сразу, как только квота появилась (см.
  // chat_screen.dart)
  final bool proAvailableNow;
  ...
  factory UsageInfo.fromJson(Map<String, dynamic> json) {
    return UsageInfo(
      ...
      proAvailableNow: json['pro_available_now'] as bool? ?? false,
    );
  }
}
