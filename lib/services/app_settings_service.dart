...
class AppSettingsService {
  ...
  Future<PublicAppSettings> updateSettings({
    ...
    if (res.statusCode >= 400) {
      throw AppSettingsException(currentL10n()?.appSettingsChangeFailed(res.statusCode) ?? 'Не удалось изменить настройку (код ${res.statusCode}).');
    }
    return PublicAppSettings.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<PublicAppSettings> resetToDefaults({
    ...
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw AppSettingsException(currentL10n()?.appSettingsRestoreFailed(res.statusCode) ?? 'Не удалось восстановить настройки (код ${res.statusCode}).');
    }
    return PublicAppSettings.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<PersonaSettings> getPersonaSettings({required String baseUrl, required String token}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw AppSettingsException(currentL10n()?.appSettingsLoadPersonaFailed(res.statusCode) ?? 'Не удалось загрузить настройки личности ИИ (код ${res.statusCode}).');
    }
  ...
  Future<PersonaSettings> updatePersonaSettings({
    ...
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw AppSettingsException(currentL10n()?.appSettingsSavePersonaFailed(res.statusCode) ?? 'Не удалось сохранить личность ИИ (код ${res.statusCode}).');
    }
    return PersonaSettings.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<PersonaSettings> resetPersonaSettings({required String baseUrl, required String token}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw AppSettingsException(currentL10n()?.appSettingsResetPersonaFailed(res.statusCode) ?? 'Не удалось сбросить личность ИИ (код ${res.statusCode}).');
    }
    return PersonaSettings.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<ModelSettings> getModelSettings({required String baseUrl, required String token}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw AppSettingsException(currentL10n()?.appSettingsLoadModelFailed(res.statusCode) ?? 'Не удалось загрузить настройки модели (код ${res.statusCode}).');
    }
  ...
  Future<ModelSettings> updateModelSettings({
    ...
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw AppSettingsException(currentL10n()?.appSettingsSaveModelFailed(res.statusCode) ?? 'Не удалось сохранить настройки модели (код ${res.statusCode}).');
    }
    return ModelSettings.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<ModelSettings> resetModelSettings({required String baseUrl, required String token}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw AppSettingsException(currentL10n()?.appSettingsResetModelFailed(res.statusCode) ?? 'Не удалось сбросить настройки модели (код ${res.statusCode}).');
    }
    return ModelSettings.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
