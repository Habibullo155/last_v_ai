...
class CustomTestsService {
  ...
  Future<List<CustomTestSummary>> list({required String baseUrl, required String token}) async {
    ...
    if (res.statusCode >= 400) {
      throw CustomTestsException(_extractError(res.body) ?? currentL10n()?.customTestsLoadListFailed ?? 'Не удалось загрузить список тестов.');
    }
  ...
  Future<CustomTest> get({required String baseUrl, required String token, required int testId}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw CustomTestsException(_extractError(res.body) ?? currentL10n()?.customTestsLoadTestFailed ?? 'Не удалось загрузить тест.');
    }
  ...
  Future<CustomTestResult> submit({
    ...
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw CustomTestsException(_extractError(res.body) ?? currentL10n()?.customTestsSubmitAnswersFailed ?? 'Не удалось отправить ответы.');
    }
  ...
  Future<List<CustomTestResultHistory>> myResults({required String baseUrl, required String token}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw CustomTestsException(_extractError(res.body) ?? currentL10n()?.customTestsLoadHistoryFailed ?? 'Не удалось загрузить историю тестов.');
    }
  ...
  Future<CustomTest> adminCreate({
    ...
    if (res.statusCode >= 400) {
      throw CustomTestsException(_extractError(res.body) ?? currentL10n()?.customTestsCreateFailed ?? 'Не удалось создать тест.');
    }
    return CustomTest.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<CustomTest> adminUpdate({
    ...
    final res = await _client
        ...
        .timeout(const Duration(seconds: 20));
    if (res.statusCode >= 400) {
      throw CustomTestsException(_extractError(res.body) ?? currentL10n()?.customTestsSaveFailed ?? 'Не удалось сохранить тест.');
    }
    return CustomTest.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<void> adminDelete({required String baseUrl, required String token, required int testId}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw CustomTestsException(_extractError(res.body) ?? currentL10n()?.customTestsDeleteFailed ?? 'Не удалось удалить тест.');
    }
  }
