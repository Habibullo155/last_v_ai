...
class BlogService {
  ...
  Future<BlogPost> getPost({required String baseUrl, required String token, required int postId}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw BlogException(_extractError(res.body) ?? currentL10n()?.blogLoadPostFailed ?? 'Не удалось загрузить пост.');
    }
  ...
  Future<BlogPost> createPost({
    ...
    if (res.statusCode >= 400) {
      throw BlogException(_extractError(res.body) ?? currentL10n()?.blogCreatePostFailed ?? 'Не удалось создать пост.');
    }
    return BlogPost.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<BlogPost> updatePost({
    ...
    final res = await _client
        ...
        .timeout(const Duration(seconds: 20));
    if (res.statusCode >= 400) {
      throw BlogException(_extractError(res.body) ?? currentL10n()?.blogSavePostFailed ?? 'Не удалось сохранить пост.');
    }
    return BlogPost.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<void> deletePost({required String baseUrl, required String token, required int postId}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw BlogException(_extractError(res.body) ?? currentL10n()?.blogDeletePostFailed ?? 'Не удалось удалить пост.');
    }
  }
  ...
  Future<BlogPost> toggleLike({required String baseUrl, required String token, required int postId}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw BlogException(_extractError(res.body) ?? currentL10n()?.blogLikeFailed ?? 'Не удалось поставить лайк.');
    }
    return BlogPost.fromJson(jsonDecode(res.body) as Map<String, dynamic>);
  }
  ...
  Future<BlogComment> addComment({
    ...
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw BlogException(_extractError(res.body) ?? currentL10n()?.blogSendCommentFailed ?? 'Не удалось отправить комментарий.');
    }
  ...
  Future<void> deleteComment({required String baseUrl, required String token, required int commentId}) async {
    final res = await _client
        ...
        .timeout(const Duration(seconds: 15));
    if (res.statusCode >= 400) {
      throw BlogException(_extractError(res.body) ?? currentL10n()?.blogDeleteCommentFailed ?? 'Не удалось удалить комментарий.');
    }
  }
